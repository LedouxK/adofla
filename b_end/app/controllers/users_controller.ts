...
export default class UsersController {
  public async changePassword({ request, response, auth }: HttpContext) {
    try {
      ...
      if (new_password !== confirm_password) {
        return response.badRequest({ message: 'New password and confirmation password do not match' });
      }
      ...
      console.error(error);
      return response.internalServerError({ message: 'Failed to change password', error: error.message });
    }
  }
  ...
  public async resetPassword({ request, response }: HttpContext) {
    ...
    // Validate token
    const tokenRecord = await ResetPassword.query()
      .where('token', token)
      .first()
